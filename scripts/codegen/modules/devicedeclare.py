...
def _device_name(device_id):
    device_name = codegen.edts().device_property(device_id, 'compatible/0', None)
    if device_name is None:
        codegen.error("No compatible property for device id '{}'."
                      .format(device_id))

    bus_master_device_id = codegen.edts().device_property(device_id, 'bus/master', None)
    if bus_master_device_id is not None:
        reg = codegen.edts().device_property(bus_master_device_id, 'reg')
        try:
    ...
    reg = codegen.edts().device_property(device_id, 'reg', None)
    if reg is None:
        # no reg property - take the reg property of the parent device
        parent_device_id = codegen.edts().device_property(
            device_id, 'parent-device', None)
        if parent_device_id:
            reg = codegen.edts().device_property(parent_device_id, 'reg', None)
    device_address = None
    if reg is not None:
        try:
            # use always the first key to get first address inserted into dict
    ...
    if device_address is None:
        ...
        device_address = codegen.edts().device_property(device_id, 'label',
                                                        '<unknown>')
...
def _device_template_substitute(template, device_id, preset={}):
    ...
    mapping.update(codegen.edts().device_properties_flattened(device_id))
...
def device_declare_single(device_config,
    ...
    device_id = codegen.edts().device_id_by_label(driver_name)
...
def device_declare(compatibles, init_prio_flag, kernel_level, irq_func,
    ...
    for device_id in codegen.edts().device_ids_by_compatible(compatibles):
        driver_name = codegen.edts().device_property(device_id, 'label')
        device_config = "CONFIG_{}".format(driver_name.strip('"'))
        interrupts = codegen.edts().device_property(
                                            device_id, 'interrupts', None)
