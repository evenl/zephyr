 ...
 * """
 * 	SPI_CONTEXT_INIT_LOCK(${device-name}_data, ctx),
 * 	SPI_CONTEXT_INIT_SYNC(${device-name}_data, ctx),
 * """
 * )
 *
 *
 ...
 * #ifdef CONFIG_SPI_STM32_INTERRUPT
 *      .irq_config = ${device-name}_config_irq,
 * #endif
 * """)
 *
 * devicedeclare.device_declare(['st,stm32-spi', 'st,stm32-spi-fifo'],
